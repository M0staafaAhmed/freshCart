...
export default async function page() {
    ...
    async function getcategorys(): Promise<categoryRes | null> {
        try {
            ...
            console.error("Fetch Error (getcategorys):", error);
            return null;
        }
    }
            ...
            <div className="w-full bg-linear-to-r from-green-600 via-green-500 to-green-400 text-white">
                <div className="container mx-auto px-4 py-12 sm:py-16">
                    ...
                    <nav className="flex items-center gap-2 text-white/80 text-sm mb-6">
                        ...
                        <span>/</span>
                        <span className="text-white font-medium">categorys</span>
                    </nav>
                    ...
                    <div className="flex items-center gap-5">
                        ...
                        <div className="flex flex-col">
                            ...
                            <p className="text-white/90 mt-1 font-medium">
                                Shop from your favorite categorys
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div className="container mx-auto py-8">
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 sm:gap-6">
                    {categorys?.data.map((category) => {
                        return <Link key={category._id} href={`categories/${category._id}`}>
                            <div className="group bg-white rounded-2xl border border-gray-100 p-4 sm:p-5 shadow-sm hover:shadow-xl hover:border-green-200 transition-all duration-300 hover:-translate-y-1">
                                <div className="aspect-square rounded-xl overflow-hidden bg-gray-50 mb-3 p-4 flex items-center justify-center relative">
                                    <Image src={category.image} alt={category.name} fill className='object-contain group-hover:scale-110 transition-transform duration-500' />
                                </div>
